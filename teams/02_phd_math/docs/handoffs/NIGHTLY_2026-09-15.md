...
- `desk-nifty-04d104ca` NIFTY `NEUTRAL` stage=`EXPIRED` outcome=`EXPIRED` still_valid=False user=SKIPPED lots=— spot=— user_pnl=— shadow_pnl=—
  - session clock / flatten — VERIFY 15:30 vs 15:40
  - user skipped — shadow paper closed for learning
- `desk-banknifty-f947f03a` BANKNIFTY `NEUTRAL` stage=`EXPIRED` outcome=`EXPIRED` still_valid=False user=SKIPPED lots=— spot=— user_pnl=— shadow_pnl=—
  - session clock / flatten — VERIFY 15:30 vs 15:40
  - user skipped — shadow paper closed for learning
- `desk-sensex-1d3bccff` SENSEX `NEUTRAL` stage=`EXPIRED` outcome=`EXPIRED` still_valid=False user=SKIPPED lots=— spot=— user_pnl=— shadow_pnl=—
  - session clock / flatten — VERIFY 15:30 vs 15:40
  - user skipped — shadow paper closed for learning
