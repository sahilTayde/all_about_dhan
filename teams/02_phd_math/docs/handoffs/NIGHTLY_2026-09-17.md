...
- `desk-nifty-2d1ad543` NIFTY `NEUTRAL` stage=`EXPIRED` outcome=`EXPIRED` still_valid=False user=SKIPPED lots=— spot=— user_pnl=— shadow_pnl=—
  - session clock / flatten — VERIFY 15:30 vs 15:40
  - user skipped — shadow paper closed for learning
- `desk-banknifty-0fbef6be` BANKNIFTY `NEUTRAL` stage=`EXPIRED` outcome=`EXPIRED` still_valid=False user=SKIPPED lots=— spot=— user_pnl=— shadow_pnl=—
  - session clock / flatten — VERIFY 15:30 vs 15:40
  - user skipped — shadow paper closed for learning
- `desk-sensex-11dc6691` SENSEX `NEUTRAL` stage=`EXPIRED` outcome=`EXPIRED` still_valid=False user=SKIPPED lots=— spot=— user_pnl=— shadow_pnl=—
  - session clock / flatten — VERIFY 15:30 vs 15:40
  - user skipped — shadow paper closed for learning
