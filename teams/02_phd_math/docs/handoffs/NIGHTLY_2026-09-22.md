...
- `desk-nifty-a3551ccc` NIFTY `NO_TRADE` stage=`EXPIRED` outcome=`EXPIRED` still_valid=False user=SKIPPED lots=— spot=— user_pnl=— shadow_pnl=—
  - session clock / flatten — VERIFY 15:30 vs 15:40
  - user skipped — shadow paper closed for learning
- `desk-banknifty-110a0aa5` BANKNIFTY `NO_TRADE` stage=`EXPIRED` outcome=`EXPIRED` still_valid=False user=SKIPPED lots=— spot=— user_pnl=— shadow_pnl=—
  - session clock / flatten — VERIFY 15:30 vs 15:40
  - user skipped — shadow paper closed for learning
- `desk-sensex-6e0e8775` SENSEX `NO_TRADE` stage=`EXPIRED` outcome=`EXPIRED` still_valid=False user=SKIPPED lots=— spot=— user_pnl=— shadow_pnl=—
  - session clock / flatten — VERIFY 15:30 vs 15:40
  - user skipped — shadow paper closed for learning
