...
| 6 | Staged signals WATCH→EARLY→CONFIRMED→IN-PROGRESS→outcomes; customer vs engine; no indicator soup on client UI | **PARTIAL** | Paper `/ws/signals` from Dhan feed (BUY CALL/PUT/HOLD). Mock UI still default without API. **UNVALIDATED**. 09 `NOTES_ONLY`. Multi-agent paper skeleton: `packages/trading_agents_india` (TradingAgents EXTERNAL Apache-2.0) — dry CE/PE/HOLD + risk veto; not a promote. |
| 7 | Dashboard: ticket SL/target, (i) legend, book, sentiment windows, `/desk` internal | **PARTIAL** | Customer mock **wired** + right-rail **agreement confidence** box ([`CUSTOMER_TICKET.md`](../teams/05_analysis/docs/CUSTOMER_TICKET.md)). Numbers labeled **MOCK** / paper. Not live Dhan fills. Confidence ≠ win rate. |
