...
| STRAT-001 | **primary** (alt book, not same ticket) | DHAN-DERIVED recipe; index-only book **`PROJECT_MIX`**; **WAITING working path** | `HAUSZx-hYdY` 41:32–01:03:10 (hourly + 5/10m CONFIRMED; **15m child alt spoken; 2h parent NOT_IN_EN**). Spoken universe **NIFTY 100 stocks**. INDEX 1m ≠ dual-TF | no — ablation `BULL_ALT_001` |
...
| STRAT-006 | **primary** (scalp alt) | DHAN-DERIVED; **WAITING working path** | `pvmvkiS1cx4` (2m EMA 10/20 + ITM 100–200 CONFIRMED). HQ `{1,5,15}` ≠ 2m. Beginner caution is **VIX > 15–16**, not a blanket ban (WEAK vs old STRAT line). Delta **WEAK** | no — ablation `BULL_ALT_006` |
