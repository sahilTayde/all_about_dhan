...
def test_divergence_blocks_when_ce_does_not_follow() -> None:
    ...
    assert "PREMIUM_DIVERGENCE" in why or "did not follow" in why or "HOLD" in why
...
def test_no_tape_stops() -> None:
    report = run_paper_tune(
        root=Path("/tmp/tv-ep-empty-root-does-not-exist-tune"),
        mix_ids=["MIX-TV-EP-005"],
        write=False,
        bars_override={"index": [], "ce": [], "pe": []},
    )
    # empty override still goes through align fail
    assert report["ok"] is False or report.get("stop") == "no tape" or not report.get("ok")
