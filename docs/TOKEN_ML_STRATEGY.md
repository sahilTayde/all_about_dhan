...
| ML-001 | KMeans (k=4) + IsolationForest **log** + **ITM observer review** | closed 1m INDEX + ITM wing vs the fill book's CE/PE claim | ALLOW / VETO / PASS (ATM or missing ITM = PASS) | shadow veto; `NO_PROMOTE`; [`ML_001_LOCAL_PATTERN.md`](../teams/04_quant/docs/ML_001_LOCAL_PATTERN.md) |
| ML-002 | OU on MIX-FORM residual + VWMA(MRR) windows 40/60/90 | 1m residual ε = r_opt − k r_idx; FOLLOW-GAP is closed 1m INDEX vs ITM wing | HOLD if FOLLOW_GAP_ITM_1M or \|z\|≥2; ATM day PASS | shadow; `NO_PROMOTE`; [`BOOK_MODEL_TUNE.md`](../teams/06_backtesting/docs/BOOK_MODEL_TUNE.md) |
...
| ML-2 | LightGBM / XGBoost **or paper stand-in `MIX-ML-GREEKS`** | richer chain + bar features (Dhan delta/IV/theta/gamma) | regime / HOLD bucket | OOS + 09 review; paper book only until then |
